...
LeRobot ``ManipulatorRobot`` opens the ports, and provides a safer replacement
for ``ManipulatorRobot.set_so100_robot_preset`` that skips the problematic
2-byte ``Maximum_Acceleration`` write.
"""
...
def configure_feetech_port(port: str, motor_ids: tuple[int, ...] = (1, 2, 3, 4, 5, 6)) -> None:
    ...
    * ``Acceleration = 254``
    ...
    try:
        # Disable status replies from write commands.  This is the main cause of
        # "Incorrect status packet" during the first GroupSyncRead.
        for mid in motor_ids:
            packet_handler.write1ByteTxOnly(port_handler, mid, _RESPONSE_STATUS_LEVEL_ADDR, 0)
        time.sleep(0.02)
        port_handler.ser.reset_input_buffer()

        # Minimise the per-motor reply delay so all 6 motors fit in the
        # GroupSyncRead timeout budget.
        for mid in motor_ids:
            packet_handler.write1ByteTxOnly(port_handler, mid, _RETURN_DELAY_TIME_ADDR, 0)
        time.sleep(0.02)
        port_handler.ser.reset_input_buffer()

        # Force position feedback to [0, resolution-1].
        for mid in motor_ids:
            phase, comm, _ = packet_handler.read1ByteTxRx(port_handler, mid, _PHASE_ADDR)
            if comm == scs.COMM_SUCCESS and (phase & 0x10):
        ...
        # Use the 1-byte Maximum_Acceleration register that matches the actual
