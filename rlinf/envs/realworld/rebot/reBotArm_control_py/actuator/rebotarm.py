...
class JointGroup:
    ...
    def __init__(
        ...
        # RobStride POS_VEL：缓存已下发的 limit_spd(0x7017)，仅在变化时重写，减少 CAN 负载
        self._rs_pv_vlim: np.ndarray = np.full(len(self._jcfgs), np.nan, dtype=np.float64)
    ...
    def send_pos_vel(
        ...
        for i in range(min(len(pos), self.num_joints)):
            jc = self._jcfgs[i]
            m = self._mm[jc.name]
            vl = float(vlim[i]) if i < len(vlim) else float(self._pv_vlim[i])
            try:
                if jc.vendor == "robstride":
                    # RobStride POS_VEL 不走 send_pos_vel：写 loc_ref(0x7016) 下发位置，
                    # limit_spd(0x7017) 仅在变化时更新（与 motorbridge cli/示例一致）。
                    if self._rs_pv_vlim[i] != vl:
                        m.robstride_write_param_f32(0x7017, abs(vl))
                        self._rs_pv_vlim[i] = vl
                    m.robstride_write_param_f32(0x7016, float(pos[i]))
                else:
                    m.send_pos_vel(float(pos[i]), vl)
            except CallError:
                pass
