...
@dataclass
class RebotArmRobotState:
    """State snapshot for the reBotArm 6-DOF robot.
    ...
    arm_joint_torque: np.ndarray = field(default_factory=lambda: np.zeros(6))
    """Joint torques ``[tau1, ..., tau6]`` in N-m."""

    tcp_pose: np.ndarray = field(default_factory=lambda: np.zeros(7))
    """End-effector pose ``[x, y, z, qx, qy, qz, qw]`` (m / quaternion).
    Computed via FK from joint positions."""
    ...
    """Gripper joint position in hardware units."""
